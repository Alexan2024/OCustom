...
# Система налогообложения в чеке. У ЮKassa поле зовётся tax_system_code,
# до кассы доезжает как sno. Без него Атол отбивает чек целиком:
# «Ошибка валидации входящего чека … Required properties ["sno"]».
# Коды (число от 1 до 6):
#   1 — ОСН                        4 — ЕНВД
#   2 — УСН «Доходы»      ← наш    5 — ЕСХН
#   3 — УСН «Доходы минус расходы» 6 — Патент
# У ÖMANKÖ — УСН «Доходы», код 2. Ставка НДС 5% (код 7 выше) этому не
# противоречит: упрощенцы с оборотом свыше порога платят НДС по 5% или 7%.
# 0 — поле не передавать (годится только если у магазина в кассе
# зарегистрирована ровно одна СНО и она подставляется сама).
YOOKASSA_TAX_SYSTEM_CODE = int(_env("YOOKASSA_TAX_SYSTEM_CODE", "2"))
...
# Третьего у ЮKassa нет: частичная предоплата и аванс не поддерживаются.
# Со слов Паши (сентябрь 2026) касса бьёт «полный расчёт»: в чеке у нас
# не футболка, а участие в воркшопе, и оно закрывается в момент оплаты.
YOOKASSA_PAYMENT_MODE = _env("YOOKASSA_PAYMENT_MODE", "full_payment")

# --- Как называется позиция в чеке ---
# Паша: в чеке должен стоять «Воркшоп ÖxПГ», а не «Футболка». Размер
# дописывается сам: «Воркшоп ÖxПГ, размер M». Меняется переменной, без
# редеплоя — за формулировки в чеке отвечает касса, а не мы.
RECEIPT_ITEM_NAME = _env("RECEIPT_ITEM_NAME", "Воркшоп ÖxПГ")
RECEIPT_EXTRA_ITEM_NAME = _env("RECEIPT_EXTRA_ITEM_NAME", "Дополнительный принт")
# Признак предмета расчёта: commodity — товар, service — услуга.
# Воркшоп по смыслу ближе к услуге, но менять только по слову Паши.
RECEIPT_ITEM_SUBJECT = _env("RECEIPT_ITEM_SUBJECT", "commodity")
...
# --- Условия возврата ---
# Футболка печатается по макету покупателя — это товар с индивидуально
# определёнными свойствами, и вернуть его без брака нельзя. Человек должен
# увидеть это до оплаты: отказ в возврате, о котором не предупредили,
# держится на честном слове. Короткая строка стоит у галочки на экране
# оформления, полная — раскрывается по «Подробнее» и приходит по /terms.
RETURN_POLICY_SHORT = _env(
    "RETURN_POLICY_SHORT",
    "Футболка печатается по моему макету. Понимаю, что вернуть или обменять "
    "её без брака нельзя.",
)
RETURN_POLICY_TEXT = _env(
    "RETURN_POLICY_TEXT",
    "Согласно российскому законодательству, футболка, изготовленная по вашему "
    "собственному (индивидуальному) дизайну, не подлежит возврату или обмену, "
    "если в ней нет брака.\n\n"
    "Это правило регламентируется ст. 26.1 Закона РФ «О защите прав "
    "потребителей» и п. 4 ст. 497 ГК РФ. По закону потребитель не вправе "
    "отказаться от товара надлежащего качества, имеющего индивидуально-"
    "определенные свойства, если указанный товар может быть использован "
    "исключительно приобретающим его человеком. Поскольку вещь создана "
    "по вашему эскизу, принту или меркам, продавец не сможет перепродать её "
    "другому покупателю.",
)
...
# Переводить ли заказ в «Передан в СДЭК» самим, как только СДЭК отчитался,
# что пакет принят. Кнопка «Сдал в СДЭК 🚚» остаётся — она просто перестаёт
# быть обязательной: сотрудник сдаёт пакет в ПВЗ и уходит, а статус
# и сообщение покупателю появляются сами. CDEK_AUTO_SHIPPED=false —
# старое поведение, статус меняется только руками.
CDEK_AUTO_SHIPPED = _flag("CDEK_AUTO_SHIPPED", "true")
...
# Фиксированная цена доставки, ₽. Одна на всю Россию, направление не влияет
# (решение Паши, сентябрь 2026): покупатель видит понятное число, сотруднику
# нечего объяснять. Где-то это дороже реального тарифа СДЭК, где-то дешевле —
# разница закрывается в среднем по заказам.
# Ставим 0 — и цена снова считается по тарифу СДЭК, как раньше.
CDEK_PRICE_PVZ = int(_env("CDEK_PRICE_PVZ", "400"))    # СДЭК, пункт выдачи
CDEK_PRICE_DOOR = int(_env("CDEK_PRICE_DOOR", "800"))  # СДЭК, курьер до двери

# Наценка на тариф СДЭК, ₽ (упаковка, время сотрудника). 0 — продаём в ноль.
# К фиксированной цене выше НЕ прибавляется: та уже конечная, а не тариф.
...
# Цвет бланка, HEX. Перед и спина показываются фотографией, а рукав — схемой:
# на фронтальном кадре он уходит под углом, и мерить по нему нельзя. Схему
# заливаем этим цветом, иначе рукав на светлом фоне вообще не читается.
# Отсюда цвет берут оба места сразу — мини-апп и картинка заказа в боте,
# так что при смене бланка правится одно значение.
# Значение по умолчанию снято пипеткой с корпуса на webapp/shirt_front.webp —
# схема рукава и фотография должны быть одного цвета, иначе видно шов.
SHIRT_COLOR = _env("SHIRT_COLOR", "#151517")

# Остатки бланков по размерам. 0 = размер скрыт.
# Это только СТАРТОВОЕ значение: при первом запуске каждое число уезжает
# в базу, и дальше остаток живёт там. Меняется он командой /stock в чате
# сотрудников, а не редеплоем — на складе футболки кончаются чаще, чем
# кто-то готов лезть в Railway. Переменную трогать больше не нужно: размер,
# уже заведённый в базе, из неё не перечитывается.
SHIRT_STOCK = {
    s.split(":")[0]: int(s.split(":")[1])
    for s in _env("SHIRT_STOCK", "S:3,M:23,L:95,XL:50").split(",")
}

# Списывать ли бланк автоматически при заказе (и возвращать при отмене) —
# ровно как с принтами. false — число остаётся просто заслонкой: заказы
# перестают приниматься на нуле, но само не уменьшается.
SHIRT_STOCK_AUTO = _flag("SHIRT_STOCK_AUTO", "true")

# С какого остатка размер считается заканчивающимся: в панели /stock он
# помечается ⚠️, и на этой отметке бот пишет в чат сотрудников.
SHIRT_STOCK_LOW = int(_env("SHIRT_STOCK_LOW", "3"))
